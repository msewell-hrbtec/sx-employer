...
const loadData = (event?: any) => {
  ...
  hrbCore.getUsersByEmployerIdAndArchivedAndRoleWithPaging(searchEmployer.value || "", searchArchived.value, searchRole.value, pagingInfo.value).then((response: any) => {
    loading.value = false
    data.value = response.data
    totalRecords.value = response.rowCount
  })
}
